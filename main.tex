...
% --- ACM venues (acmart) ---
% Camera-ready: no review/anonymous options
% Review: add review,anonymous for double-blind submission
\ifx\targetconference\confasplos
    \ifcameraready \documentclass[sigplan,10pt]{acmart}
    \else          \documentclass[sigplan,10pt,review,anonymous]{acmart}
    \fi
\fi
\ifx\targetconference\confsosp
    \ifcameraready \documentclass[nonacm,sigplan,10pt]{acmart}
    \else          \documentclass[nonacm,sigplan,10pt,review,anonymous]{acmart}
    \fi
\fi
...
\ifx\targetconference\confsocc
    \ifcameraready \documentclass[sigconf,10pt]{acmart}
    \else          \documentclass[sigconf,10pt,review,anonymous]{acmart}
    \fi
\fi
\ifx\targetconference\confvldb
    \ifcameraready \documentclass[sigconf,nonacm]{acmart}
    \else          \documentclass[sigconf,nonacm,review,anonymous]{acmart}
    \fi
\fi
\ifx\targetconference\confsigmod
    \ifcameraready \documentclass[sigconf]{acmart}
    \else          \documentclass[sigconf,review,anonymous]{acmart}
    \fi
\fi
...
% --- MLSys ---
% Camera-ready: [accepted] shows author names
% Review: (no option) shows "Under review" + line numbers
\ifx\targetconference\confmlsys
    \ifcameraready \usepackage[accepted]{mlsys2024}
    \else          \usepackage{mlsys2024}
    \fi
\fi

% --- NeurIPS ---
% Camera-ready: [final] shows author names
% Review: (no option) anonymous with line numbers
\ifx\targetconference\confneurips
    \ifcameraready \usepackage[final]{neurips_2025}
    \else          \usepackage{neurips_2025}
    \fi
\fi

% --- COLM ---
% Camera-ready: [final] shows author names
% Review: [submission] enables anonymization
\ifx\targetconference\confcolm
    \ifcameraready \usepackage[final]{colm2025_conference}
    \else          \usepackage[submission]{colm2025_conference}
    \fi
\fi
...
\newcommand{\renderauthorfn}{%
  ...
  \ifdefined\authorfnF\quad\authorfnF\fi
  \ifdefined\authorfnG\quad\authorfnG\fi
  \ifdefined\authorfnH\quad\authorfnH\fi
  \ifdefined\authorfnI\quad\authorfnI\fi
  \ifdefined\authorfnJ\quad\authorfnJ\fi
  \ifdefined\authorfnK\quad\authorfnK\fi
  \ifdefined\authorfnL\quad\authorfnL\fi
  \ifdefined\authorfnM\quad\authorfnM\fi
  \ifdefined\authorfnN\quad\authorfnN\fi
  \ifdefined\authorfnO\quad\authorfnO\fi
  \ifdefined\authorfnP\quad\authorfnP\fi
  \ifdefined\authorfnQ\quad\authorfnQ\fi
  \ifdefined\authorfnR\quad\authorfnR\fi
  \ifdefined\authorfnS\quad\authorfnS\fi
  \ifdefined\authorfnT\quad\authorfnT\fi
  \ifdefined\authorfnU\quad\authorfnU\fi
  \ifdefined\authorfnV\quad\authorfnV\fi
  \ifdefined\authorfnW\quad\authorfnW\fi
  \ifdefined\authorfnX\quad\authorfnX\fi
  \ifdefined\authorfnY\quad\authorfnY\fi
  \ifdefined\authorfnZ\quad\authorfnZ\fi
}
...
\newcommand{\condensedauthorlist}{%
  ...
  \addauthor{K}\addauthor{L}\addauthor{M}\addauthor{N}\addauthor{O}%
  \addauthor{P}\addauthor{Q}\addauthor{R}\addauthor{S}\addauthor{T}%
  \addauthor{U}\addauthor{V}\addauthor{W}\addauthor{X}\addauthor{Y}%
  \addauthor{Z}%
}
...
\newcommand{\instlegend}{%
  ...
  \addinstlegend{K}\addinstlegend{L}\addinstlegend{M}\addinstlegend{N}\addinstlegend{O}%
  \addinstlegend{P}\addinstlegend{Q}\addinstlegend{R}\addinstlegend{S}\addinstlegend{T}%
  \addinstlegend{U}\addinstlegend{V}\addinstlegend{W}\addinstlegend{X}\addinstlegend{Y}%
  \addinstlegend{Z}%
}
\makeatother
...
\ifacmvenue
    \ifcameraready
        % Camera-ready: uses macros from config.tex if defined
        % \expandafter needed: \setcopyright uses xkeyval which doesn't expand macros
        \ifdefined\papercopyright \expandafter\setcopyright\expandafter{\papercopyright} \fi
        \ifdefined\paperdoi \acmDOI{\paperdoi} \fi
        \ifdefined\paperisbn \acmISBN{\paperisbn} \fi
        \ifdefined\confyear \acmYear{\confyear} \fi
        \ifdefined\confname
            \acmConference[\confshort]{\confname}{\confdate}{\conflocation}
        \fi
    \else
...
\ifx\targetconference\confmlsys
    ...
    \begin{mlsysauthorlist}
        ...
        \ifdefined\authorKname \mlsysauthor{\getname{K}}{aff\authorKinst} \fi
        \ifdefined\authorLname \mlsysauthor{\getname{L}}{aff\authorLinst} \fi
        \ifdefined\authorMname \mlsysauthor{\getname{M}}{aff\authorMinst} \fi
        \ifdefined\authorNname \mlsysauthor{\getname{N}}{aff\authorNinst} \fi
        \ifdefined\authorOname \mlsysauthor{\getname{O}}{aff\authorOinst} \fi
        \ifdefined\authorPname \mlsysauthor{\getname{P}}{aff\authorPinst} \fi
        \ifdefined\authorQname \mlsysauthor{\getname{Q}}{aff\authorQinst} \fi
        \ifdefined\authorRname \mlsysauthor{\getname{R}}{aff\authorRinst} \fi
        \ifdefined\authorSname \mlsysauthor{\getname{S}}{aff\authorSinst} \fi
        \ifdefined\authorTname \mlsysauthor{\getname{T}}{aff\authorTinst} \fi
        \ifdefined\authorUname \mlsysauthor{\getname{U}}{aff\authorUinst} \fi
        \ifdefined\authorVname \mlsysauthor{\getname{V}}{aff\authorVinst} \fi
        \ifdefined\authorWname \mlsysauthor{\getname{W}}{aff\authorWinst} \fi
        \ifdefined\authorXname \mlsysauthor{\getname{X}}{aff\authorXinst} \fi
        \ifdefined\authorYname \mlsysauthor{\getname{Y}}{aff\authorYinst} \fi
        \ifdefined\authorZname \mlsysauthor{\getname{Z}}{aff\authorZinst} \fi
    \end{mlsysauthorlist}
    ...
    \ifdefined\instK \mlsysaffiliation{affK}{\instK} \fi
    \ifdefined\instL \mlsysaffiliation{affL}{\instL} \fi
    \ifdefined\instM \mlsysaffiliation{affM}{\instM} \fi
    \ifdefined\instN \mlsysaffiliation{affN}{\instN} \fi
    \ifdefined\instO \mlsysaffiliation{affO}{\instO} \fi
    \ifdefined\instP \mlsysaffiliation{affP}{\instP} \fi
    \ifdefined\instQ \mlsysaffiliation{affQ}{\instQ} \fi
    \ifdefined\instR \mlsysaffiliation{affR}{\instR} \fi
    \ifdefined\instS \mlsysaffiliation{affS}{\instS} \fi
    \ifdefined\instT \mlsysaffiliation{affT}{\instT} \fi
    \ifdefined\instU \mlsysaffiliation{affU}{\instU} \fi
    \ifdefined\instV \mlsysaffiliation{affV}{\instV} \fi
    \ifdefined\instW \mlsysaffiliation{affW}{\instW} \fi
    \ifdefined\instX \mlsysaffiliation{affX}{\instX} \fi
    \ifdefined\instY \mlsysaffiliation{affY}{\instY} \fi
    \ifdefined\instZ \mlsysaffiliation{affZ}{\instZ} \fi
